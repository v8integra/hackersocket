...
const AddEducation = ({ addEducation, history }) => {
  ...
  const { school, degree, fieldofstudy, from, to, current, description } = formDatae;

  const onChange = e => setFormData({ ...formDatae, [e.target.name]: e.target.value });
  ...
  return (
    <Fragment>
      ...
      <form class="form" onSubmit={e => {
        e.preventDefault();
        addEducation(formDatae, history);
      }}>
        ...
        <div class="form-group">
         ...
         <div class="form-group">
          <p><input type="checkbox" name="current" checked={ current } value={current} 
            onChange={e => { 
                setFormData({ ...formDatae, current: !current }); 
